                      ...
                      <form method="post" action="insert_tpa.php" enctype="multipart/form-data">
                          ...
                          <div class="form-group col-md-12">
                              <div class="alert alert-info">
                                  <i class="fa fa-info-circle"></i> PTS Yang Ditampilkan adalah PTS yang belum dinilai...
                              </div>
                              <label for="nama_pts">Nama Perguruan Tinggi Swasta</label>
                                  <select required class="form-control" name="id_calon_kr">
                                  <?php  foreach ($db->select('*','pts')->where('id_calon_kr not in (select id_calon_kr from hasil_tpa)')->get() as $val): ?> 
                                  <option value="<?= $val['id_calon_kr']?>"><?= $val['nama_pts'] ?></option>
                                  <?php endforeach ?>
                                  </select>
                          </div>
